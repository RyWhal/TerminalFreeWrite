...
display_image = display_image.rotate(270)
display_draw = ImageDraw.Draw(display_image)

#Display settings like font size, spacing, etc.
display_start_line = 0
font24 = ImageFont.truetype('Courier Prime.ttf', 11) #24
textWidth=11
linespacing = 22
chars_per_line = 28 #28
lines_on_screen = 6
...
def update_display():
    ...
    display_draw.rectangle((0, 0, epd.height, epd.width), fill=255)
    ...
    if console_message != "":
        display_draw.rectangle((10, 10, epd.height, epd.width), fill=255)
        display_draw.text((10, 10), console_message, font=font24, fill=0)
        console_message = ""
...
def update_input_area(): #this updates the input area of the typewriter (active line)
    ...
    display_draw.rectangle((0, 98, epd.height, epd.width), fill=255)  # Clear display
    
    #add cursor
    temp_content = input_content[:cursor_index] + "|" + input_content[cursor_index:]
    
    #draw input line text
    display_draw.text((10, 98), str(temp_content), font=font24, fill=0)
...
def handle_key_press(e):
    ...
    if e.name == "esc" and control_active: #ctrl+esc
        ...
        display_draw.rectangle((0, 0, epd.height, epd.width), fill=255)  # Clear display
        display_draw.text((25, 64), "ZeroWriter Powered Down.", font=font24, fill=0)
        partial_buffer = epd.getbuffer(display_image)
        epd.display(partial_buffer)
        time.sleep(3)
